...
export const HistorySection = () => {
  ...
  const getSentimentColor = (sentiment: string) => {
    if (sentiment === "POSITIVE") return "bg-green-500/40";
    if (sentiment === "NEGATIVE") return "bg-red-500/40";
    return "bg-slate-500/40";
  };

  return (
    <Card className="bg-white/5 border border-slate-700/20 backdrop-blur-xl h-full shadow-2xl transition-all duration-300 rounded-lg">
      <CardHeader className="flex justify-between items-center p-2 gap-1 border-b border-slate-700/20">
        <div className="flex gap-2 items-center min-w-0">
          <div className="bg-white/10 p-1 rounded-lg shadow-lg shrink-0 transition-transform duration-200 hover:scale-105 border border-slate-600/20">
            <History className="text-white" size={16} />
          </div>
          <div className="min-w-0">
            <p className="text-sm font-semibold text-white truncate">Prediction History</p>
      ...
      <CardBody className="px-2 pb-2 pt-2 flex flex-col h-full gap-2">
        <ScrollShadow className="flex-1 max-h-[350px] min-h-[300px]">
              ...
              {isLoading ? (
                ...
                historyData?.predictions.map((prediction, index) => (
                  <div
                    ...
                    className={`bg-white/5 border border-slate-700/20 p-2 rounded-lg transition-all duration-200 hover:bg-white/10 hover:border-slate-600/30 group ${
                      index > 0 ? "mt-1" : ""
                    }`}
                  >
                    <div className="flex justify-between items-center mb-2 gap-2">
                      <div className="flex items-center gap-2 min-w-0 flex-1">
                        <div
                          className={`w-1 h-6 rounded-full shrink-0 transition-all ${getSentimentColor(
                        ...
                        <Chip
                          variant="flat"
                          size="sm"
                          className="font-semibold text-[8px] uppercase bg-white/10 text-white border border-slate-600/20"
                        >
                          {prediction.sentiment}
                        </Chip>
                      </div>
                      <div className="flex items-center gap-1 text-[8px] text-slate-400 shrink-0">
                        <Clock size={8} />
